...
```
长按事件设置与basestatusbar的getNotificationLongClicker()
                            ...
                            mWatchLongPress = new Runnable() {
                                ...
                                public void run() {
                                    if (mCurrView != null && !mLongPressSent) {
                                        ...
                                        // x和y是中心坐标值，回调出去可以利用NotificationGuts产生水波纹的效果
                                    }
                                }
                            };
                        }
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_MOVE:
                if (mCurrView != null) {
                    ...
                    setTranslation(mCurrView, mTranslation + delta); // 实时移动
                    // 两件事：一是设置透明度，而是invalidate the view's own bounds，invalidate(int l, int t, int r, int b)
                    updateSwipeProgressFromOffset(mCurrView, mCanCurrViewBeDimissed);
                    onMoveUpdate(mCurrView, mTranslation + delta, delta);//空函数。供子类
                }
                break;
            case MotionEvent.ACTION_UP:
            case MotionEvent.ACTION_CANCEL:
                ...
                if (!handleUpEvent(ev, mCurrView, velocity, getTranslation(mCurrView))) {//空函数。供子类
                    if (isDismissGesture(ev)) { // 判断是否dismiss
                        // flingadingy
                        ...
                        snapChild(mCurrView, 0 /* leftTarget */, velocity);//位置为0即偏移0，即恢复
                    }
                    mCurrView = null;
                }
                mDragging = false;
...
```
    ...
    protected boolean isDismissGesture(MotionEvent ev) {
        ...
        return !falsingDetected && (swipedFastEnough() || swipedFarEnough())
                ...
                && mCallback.canChildBeDismissed(mCurrView);//多重判断！！
    }
    
    protected boolean swipedFarEnough() {//距离
        float translation = getTranslation(mCurrView);
        return DISMISS_IF_SWIPED_FAR_ENOUGH && Math.abs(translation) > 0.4 * getSize(mCurrView);
    }

    protected boolean swipedFastEnough() {//速度
        float velocity = getVelocity(mVelocityTracker);
        float translation = getTranslation(mCurrView);
    ...
    private void updateSwipeProgressFromOffset(View animView, boolean dismissable,
        ...
        if (!mCallback.updateSwipeProgress(animView, dismissable, swipeProgress)) {
            if (FADE_OUT_DURING_SWIPE && dismissable) { //设置滑动的透明度
    ...
    public static void invalidateGlobalRegion(View view, RectF childBounds) {
        ...
        while (view.getParent() != null && view.getParent() instanceof View) {
            ...
            view.invalidate((int) Math.floor(childBounds.left),
                            ...
                            (int) Math.ceil(childBounds.bottom));//invalidate！！！！
            if (DEBUG_INVALIDATE) {
...
```
    ...
    public void dismissChild(final View animView, float velocity, final Runnable endAction,
        ...
        boolean animateLeft = velocity < 0
                || (velocity == 0 && getTranslation(animView) < 0 && !isDismissAll);//左还是上

        if (animateLeft || animateLeftForRtl || animateUpForMenu) {
            newPos = -getSize(animView);//左则负值，完全消失的位置
        } else {
    ...
    private void snapChildInstantly(final View view) {//立刻
